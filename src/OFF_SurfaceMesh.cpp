...
std::unique_ptr<SurfaceMesh> readOFFm(const std::string &filename)
{
    ...
    for (int i = 0; i < numFaces; i++)
    {
        ...
        else if (arr.size() == dimension+2)
        {
            auto v0 = std::stoi(arr[1]);
            auto v1 = std::stoi(arr[2]);
            auto v2 = std::stoi(arr[3]);
            int m = std::stoi(arr[4]);
            mesh->insert<3>({v0, v1, v2}, SMFace(m, false));
        }
        else if (arr.size() == dimension+5)
        {
            auto v0 = std::stoi(arr[1]);
...
void writeOFFm(const std::string &filename, const SurfaceMesh &mesh)
{
    ...
    {
        auto w = mesh.get_name(faceNodeID);
        int marker = (*faceNodeID).marker;
        auto orientation = (*faceNodeID).orientation;
        if (orientation == 1)
        {
            fout << "3 " << sigma[w[0]] << " " << sigma[w[1]] << " " << sigma[w[2]] << " " << marker << "\n";
        }
        else if (orientation == -1)
        {
            fout << "3 " << sigma[w[2]] << " " << sigma[w[1]] << " " << sigma[w[0]] << " " << marker << "\n";
